...
mod tests {
    ...
    fn test_odlyzko_cache_determinism() {
        ...
        assert_eq!(
            cache_a.zeros, cache_b.zeros,
            "Same cache must produce same zeros"
        );
        assert_eq!(
            cache_a.hash, cache_b.hash,
            "Same cache must produce same hash"
        );
    }
    ...
    fn test_validator_runs_without_error() {
        let report =
            validator::validate_first_10().expect("Validator must not error with available cache");
