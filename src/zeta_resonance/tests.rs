...
fn calibrate_for_workload(workload: Workload) -> PowerModel {
    ...
    // Ground truth: physical meter integrates V·I with ADC quantization + jitter
    ...
    // PMU counter totals for this workload
    ...
    // Scale factor: maps raw counter sum to ground truth energy.
    // estimate_energy() does: total += ema_power * dt for each sample.
    // For constant power: total_energy ≈ scale * total_raw * dt.
    // Therefore: scale = meter_energy / (total_raw * dt).
...
// --- Multi-Workload Drift Invariant (Calibrated Per Workload) ---
