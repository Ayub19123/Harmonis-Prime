...
impl MedianOfNReporter {
    ...
    pub fn report(streams: &[CognitiveStream], window_size: usize) -> MeshReport {
        let mut all_values: Vec<f64> = Vec::new();
        let mut all_measurements: Vec<Measurement> = Vec::new();

        for stream in streams.iter() {
            let window = stream.measurements.iter().rev().take(window_size).cloned();
        ...
        let outliers: Vec<Measurement> = all_measurements
            .iter()
            .filter(|m| (m.value - median).abs() > threshold)
            .cloned()
            .collect();

        let byzantine_detected =
            !all_values.is_empty() && (outliers.len() as f64 / all_values.len() as f64) > 0.30;
