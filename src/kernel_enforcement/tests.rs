...
use super::netfilter::{NetfilterPolicy, NetfilterAction};
...
fn test_ebpf_airgap_drops_all_packets_when_active() {
    ...
    assert_eq!(result, KernelFilterResult::Drop("eBPF airgap: all egress denied at kernel level"));
}
...
fn test_syscall_filter_blocks_connect() {
    ...
    assert_eq!(result, SyscallResult::Block("Airgap: network egress blocked at syscall layer"));
}
