...
use crate::mesh::dag::{Message, MessageId, NodeId, DagError};
...
impl ClusterSimulation {
    ...
    pub fn run(&mut self) -> Result<ClusterReport, DagError> {
                ...
                match node.process_message(msg) {
                    Ok((receipt, telem)) => {
                        ...
                        if streams.len() <= source_id.0 as usize {
                            streams.resize((source_id.0 + 1) as usize, CognitiveStream {
                                stream_id: source_id.0,
                                measurements: Vec::new(),
                            });
                        }
                        streams[source_id.0 as usize].measurements.push(Measurement {
                            value: stream_value,
                            timestamp: i as u64,
        ...
        let latencies: Vec<f64> = self.telemetry.iter().map(|t| t.latency_micros as f64).collect();
        ...
        let direct_byzantine_detected = {
            ...
            if stream_means.len() < 2 {
                ...
                stream_means.iter().any(|&m| global_median > 0.0 && m > global_median * 10.0)
            }
        };
        ...
        Ok(ClusterReport {
            ...
            byzantine_nodes: (self.config.node_count as f64 * self.config.byzantine_ratio).floor() as usize,
            offline_nodes: (self.config.node_count as f64 * self.config.offline_ratio).floor() as usize,
            total_messages,
