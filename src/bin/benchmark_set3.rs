...
use sovereign_core::energy::monitor::{EnergyMonitor, JloReport, SoftwareEnergyMonitor};
use sovereign_core::energy::rapl_bindings::{RaplDomain, RaplHardwareMonitor}; // <-- ADDED
...
fn main() {
    ...
    for i in 0..iterations {
        ...
        let parents: Vec<MessageId> = (0..parent_count).map(|p| MessageId(p as u64)).collect();
    ...
    println!(
        "Avg latency: {:.2} us",
        metrics.avg_insertion_latency_micros
    );
    ...
    println!(
        "Joules per operation: {:.6e} J/op",
        energy_report.joules_per_op
    );
    println!(
        "Thermal efficiency: {:.2} ops/J",
        energy_report.thermal_efficiency
    );
    ...
    println!(
        ...
        entropy_tracker.verify_landauer_limit()
    );
    ...
    println!(
        "Theoretical minimum ({} bits x {:.2e} J/bit): {:.6e} J/op",
