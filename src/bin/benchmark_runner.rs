...
use sovereign_core::benchmark::comparator::{BaselineComparator, par2_score};
...
struct Cli {
    ...
    #[arg(short, long, default_value = "benchmark_output", help = "Output directory")]
    output_dir: PathBuf,

    #[arg(short, long, default_value = "json", help = "Output format: json or csv")]
    format: String,
    ...
    #[arg(short, long, default_value_t = 300, help = "Timeout per instance in seconds")]
    timeout: u64,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Total instances: {}", successful_runs.len() + failed_count);
    ...
    if let Some(db_path) = db_path_opt {
        ...
        println!("  Recorded {} runs to {}", successful_runs.len(), db_path.display());
    }
    ...
    if let Some(baseline_tag) = cli.baseline_tag {
        if let Some(db_path) = db_path_opt {
            ...
            let current: HashMap<String, u64> = successful_runs
                .iter()
                .map(|r| (r.instance_path.to_string_lossy().to_string(), r.wall_time_ms))
                .collect();
